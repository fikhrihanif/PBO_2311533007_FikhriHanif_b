...
public class OrderDetailRepo implements OrderDetailDAO{
	...
	final String insert = "INSERT INTO orderdetail (order_id, service_id, harga, jumlah, total) VALUES (?,?,?,?,?);";
	final String delete = "DELETE FROM orderdetail WHERE id=?;";
	final String update = "UPDATE orderdetail SET order_id=?, service_id=?, harga=?, jumlah=?, total=? WHERE id=?;";
	...
	public String total(String order_id) {
		// TODO Auto-generated method stub
		String query_total = "SELECT sum(total) as total FROM orderdetail WHERE order_id="+"'"+order_id+"'"+";";
		Statement st;
		ResultSet rs;
		String result="";
		try {
